...
sess = tf.Session("grpc://" + os.environ['MASTER_IP'] + ":" + os.environ['MASTER_PORT'])
...
def result_print():
    ...
    #publish.single("lab/mq5", float("%.2f" % mq5_average), hostname=BROKER)
    #publish.single("lab/mq7", float("%.2f" % mq7_average), hostname=BROKER)
    #publish.single("lab/mq131", float("%.2f" % mq131_average), hostname=BROKER)
    #publish.single("lab/mq135", float("%.2f" % mq135_average), hostname=BROKER)

    message = '{"request":{"number":%s,"application":"scale"},"output":{"timestamp":%s,"result":{"mq5":%.2f,"mq7":%.2f,"mq131":%.2f,"mq135":%.2f}}}' % (os.environ['REQUEST'],time.time(),mq5_average,mq7_average,mq131_average,mq135_average)
    publish.single("lab/mq",message,hostname=BROKER)
